...
function result() {
	...
	if ( eval(inputCalc.value) == Infinity){
		resultCalc.value = '0,00' ;
	   inputCalc.value = 'Vous avez cassé la calculatrice.' ; //На ноль не делиться!!! 
	}
  if ( eval(inputCalc.value) === 0.016666666666666666 && inputCalc.value === "3/5/6/6"){
     //resultCalc.value = '540' ;
     inputCalc.value = 'compétence est reçue: tu es un fou' ; //На ноль не делиться!!! 
  }
  if ( eval(inputCalc.value) === -14 && inputCalc.value === "3-5-6-6"){
     //resultCalc.value = '540' ;
     inputCalc.value = 'skill: # tu es mon originalité' ; //На ноль не делиться!!! 
  }
  if ( eval(inputCalc.value) === 540 && inputCalc.value === "3*5*6*6"){
     //resultCalc.value = '540' ;
     inputCalc.value = 'Secret : #sociopathe très actif' ; //На ноль не делиться!!! 
  }
  if ( eval(inputCalc.value) === 3566){
     //resultCalc.value = '540' ;
     inputCalc.value = 'mettre un signe entre eux' ; //На ноль не делиться!!! 
  }
